    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['items_by_size'].keys():
                ...
                messages.success(
                    request,
                    f'Updated {product.title}, size {size.upper()} '
...
def adjust_bag(request, item_id):
    ...
    if size:
        if quantity > 0:
            ...
            messages.success(
                request,
                f'Updated {product.title}, size {size.upper()} quantity to '
            ...
            messages.success(
                request,
                f'Removed {product.title}, size {size.upper()} from your bag'
            )
    else:
        if quantity > 0:
            bag[item_id] = quantity
            messages.success(
                request, f'Updated {product.title} quantity to {bag[item_id]}')
        else:
            bag.pop(item_id)
